...
class InstagramService:
    ...
    def post_boat(self, boat) -> str:
        ...
        primary = next((p for p in boat.photos if p.is_primary), None) or (
            boat.photos[0] if boat.photos else None
        )
        if not primary:
            raise InstagramError(
        ...
        container_id = self._create_container(primary.url, caption)
